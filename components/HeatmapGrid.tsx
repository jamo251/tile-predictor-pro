  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 sm:p-12 glass-panel rounded-[40px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] border border-white/5">
      <p className="text-[11px] text-slate-500 text-center max-w-lg mb-6 leading-relaxed">
        ...
        <span className="text-slate-400 font-mono">{highValueThreshold}</span> (your high-score threshold). Colors are
        grouped into bands from low to strongest pattern.
      </p>
      <div
          ...
          Array.from({ length: cols }).map((_, c) => {
            ...
            return (
              ...
              >
                ...
                <span className={`absolute top-2 left-3 text-[10px] font-bold opacity-40`}>{coord}</span>
      ...
      <div className="flex flex-wrap justify-center gap-6 mt-12 text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-white/5 px-8 py-4 rounded-full border border-white/5">
        ...
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 bg-cyan-400 rounded-full shadow-[0_0_8px_rgba(34,211,238,0.5)]"></div> High heat (75–95%)
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 gemini-gradient rounded-full shadow-[0_0_10px_rgba(181,123,255,0.6)]"></div> Strongest (≥95%)
        </div>
      </div>
    </div>
  );
